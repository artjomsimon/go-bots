...
func sparselu_par_call(BENCH []*[][]float32) {
	...
	for kk := 0; kk < matrixSize; kk++ {
		waitIds1 := make([]TaskId, matrixSize)
		waitIds2 := make([]TaskId, matrixSize)
		...
		for jj := kk + 1; jj < matrixSize; jj++ {
			...
			//	fmt.Println("adding to ", jj-1)
			waitIds1[jj-1] = pool.AddTask(func() {
		...
		for ii := kk + 1; ii < matrixSize; ii++ {
			ii := ii
			kk := kk
			waitIds2[ii-1] = pool.AddTask(func() {
		...
		pool.WaitForTasks(waitIds1)
		pool.WaitForTasks(waitIds2)

		for ii := kk + 1; ii < matrixSize; ii++ {
			if BENCH[ii*matrixSize+kk] != nil {
				waitIds3 := make([]TaskId, matrixSize)
				for jj := kk + 1; jj < matrixSize; jj++ {
					if BENCH[kk*matrixSize+jj] != nil {
						...
						waitIds3[jj-1] = pool.AddTask(func() {
